...
void 
s1613par(SISLCurve *pc,double aepsge,double **gpar,int *jnbpar,int *jstat)
#else
void s1613(pc,aepsge,gpar,jnbpar,jstat)
     SISLCurve  *pc;
     double aepsge;
     double **gpar;
     int    *jnbpar;
...
*
* 
* OUTPUT     : gpar     - Array containing the parameters.
*                         Array are allocated inside this function,
*              jnbpar - Number of sampling parameters.
...
*/
{
  ...
  if (korder == 1)
    {
      kpar = kncoef;
      if ((*gpar = newarray(kpar,DOUBLE)) == SISL_NULL) goto err101;
      memcopy(*gpar,pc->et,kpar,DOUBLE);
      *jnbpar = kpar;
      *jstat = 0;
      goto out;
    }
